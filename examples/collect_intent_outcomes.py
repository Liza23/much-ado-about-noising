...
def get_inner_lowdim(envs):
    ...
    Identified by having obs_keys as a direct instance attribute (not via __getattr__).
    """
    e = envs.envs[0]
    while e is not None:
        if 'obs_keys' in e.__dict__:
            return e
        e = getattr(e, 'env', None)
    raise RuntimeError("Could not find inner RobomimicLowdimWrapper in env chain")
...
def render_frame(envs):
    """Render an RGB frame from the underlying robomimic wrapper.

    MultiStepWrapper sits above VideoRecordingWrapper, whose render() returns the
    output video path (or None) rather than pixel data. We therefore bypass that
    layer and render from the wrapped robomimic env directly.
    """
    top = get_lowdim_wrapper(envs)
    # Expected chain: MultiStepWrapper -> VideoRecordingWrapper -> Robomimic*Wrapper
    maybe_video = getattr(top, "env", None)
    maybe_base = getattr(maybe_video, "env", None)
    if maybe_base is not None and hasattr(maybe_base, "render"):
        frame = maybe_base.render(mode="rgb_array")
    else:
        # Fallback for unexpected wrapper stacks.
        frame = top.render(mode="rgb_array")
