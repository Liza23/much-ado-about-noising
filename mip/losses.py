...
def flow_loss(
    ...
    # Note: draft loss (VLA two-stage) is now added in agent.update_impl
    # so it works with ALL loss types, not just flow_loss
    return loss, {}
...
def vla_mip_loss(
    ...
    """VLA-aware MIP loss: single decoder pass per training step.

    Exploits the VLA two-stage architecture:
    - Step 1 (draft head): lightweight cross-attn predicts coarse action → render future.
      Supervised via net._draft_loss (added in update_impl), NOT a full decoder pass.
    - Step 2 (decoder): refines using rendered future + current image.
      This is the only full decoder pass.

    Compared to standard MIP (2 decoder passes), this saves ~50% decoder compute
    during training while maintaining the same predict-render-refine paradigm.

    At inference, the standard mip_sampler is used (2 decoder passes), since
    the draft head alone doesn't produce a full action trajectory.
    """
    t = torch.zeros_like(delta_t, device=delta_t.device) + config.t_two_step
    noise = torch.empty_like(act).normal_(0, 1)
    act_t = act + (1 - config.t_two_step) * noise
    ...
    # Single decoder pass for step 2 (step 1 = draft head, loss added in update_impl)
    # This call builds the cache (CLIP encode → draft → render → encode future)
    # then runs the decoder once with the noisy GT action
    act_pred = flow_map.get_velocity(t, act_t, obs_emb)
