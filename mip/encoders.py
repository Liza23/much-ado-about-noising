...
class IdentityEncoder(BaseEncoder):
    ...
    def forward(self, condition: torch.Tensor | dict, mask: torch.Tensor = None):
        ...
        if isinstance(condition, dict) or hasattr(condition, 'keys'):
            # Check if this is image observations (has 'image' key)
            try:
                if 'image' in condition:
                    # For image observations, return as-is (no flattening)
                    # Expected format: [B, To, C, H, W]
                    image_tensor = condition['image']
                    # Convert TensorDict to regular tensor if needed
                    if hasattr(image_tensor, 'to_tensor'):
                        image_tensor = image_tensor.to_tensor()
                    # No masking for images (incompatible with image format)
                    return image_tensor
            except (KeyError, TypeError) as e:
                pass

            # For non-image dict, concatenate all tensors
        ...
        # Apply mask (only for non-image observations)
        mask = at_least_ndim(
            get_mask(
                mask,
                (condition.shape[0],),
    ...
    def multi_image_forward(self, obs_dict):
        ...
        for key in self.low_dim_keys:
            ...
            if self.use_seq:
                ...
                # (e.g. intent tensors created via .expand() without .contiguous())
