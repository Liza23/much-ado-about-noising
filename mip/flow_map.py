...
class FlowMap(nn.Module):
    ...
    def __deepcopy__(self, memo):
        """Custom deepcopy that properly handles network's __deepcopy__."""
        # Deepcopy the network
        if hasattr(self.net, '__deepcopy__'):
            net_copy = self.net.__deepcopy__(memo)
        else:
            net_copy = copy.deepcopy(self.net, memo)

        # Deepcopy reference net if exists
        if self.reference_net is not None:
            if hasattr(self.reference_net, '__deepcopy__'):
                ref_net_copy = self.reference_net.__deepcopy__(memo)
            else:
                ref_net_copy = copy.deepcopy(self.reference_net, memo)
        else:
            ref_net_copy = None

        # Create new FlowMap with copied networks (this calls __init__)
        result = self.__class__(net_copy, ref_net_copy)
        memo[id(self)] = result
